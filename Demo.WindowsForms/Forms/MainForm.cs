...
//using GMap.NET.WindowsForms.Markers.GMarkerGoogle.;
...
namespace Demo.WindowsForms
{
   public partial class MainForm : Form
   {
     ...
     // readonly SQLiteIpCache IpCache = new SQLiteIpCache();
      ...
      {
          RoutingProvider rt = MainMap.MapProvider as RoutingProvider;
          if (rt == null)
          {
              rt = GMapProviders.OpenStreetMap;
              
          }

          MapRoute route = rt.GetRoute(start, end, false, false, (int) MainMap.Zoom);
          if (route != null)
          {
              // add route
              ...
              // GMapMarkerhttp://www.cyberforum.ru/csharp-beginners/thread1262170.html m3 = new GMarkerGoogle(end, GMarkerGoogleType.red_big_stop);
            //  m3.ToolTipText = "End: " + end.ToString();
      ...
      private void button3_Click(object sender, EventArgs e)
      {
       ...
       //  RoutingProvider rm = MainMap.MapProvider as RoutingProvider;
         ...
         //MapRoute ret = rm.GetRoute(start, end, true, false, (int)MainMap.Zoom);
         //if (ret != null)
         //{
         //    // add route
         //    GMapRoute r2 = new GMapRoute(ret.Points, ret.Name);
         ...
         //}
          ...
          //---------------------------------------
                    ...
                    ////////    {
                    ////////        tochki.Add(MainMap.Position);
                    ////////    }
                    ////////    start = MainMap.Position;
                    ////////    end = currentMarker.Position;
             ///--------------------------------
                        MapRoute route = rp.GetRoute(start, end, false, true, (int) MainMap.Zoom);
                        ...
                        if (route != null)
                        {
                            ...
                            // add route start/end marks
                            GMapMarker m1 = new GMarkerGoogle(MainMap.Position, GMarkerGoogleType.green_big_go);
                            m1.ToolTipText = "Start: " + route.Name;
                            m1.ToolTipMode = MarkerTooltipMode.Always;

                            GMapMarker m2 = new GMarkerGoogle(MainMap.Position, GMarkerGoogleType.red_big_stop);
                            m2.ToolTipText = "End: " + end.ToString();
                            m2.ToolTipMode = MarkerTooltipMode.Always;
      ...
      private void button19_Click(object sender, EventArgs e)// проверка построения по координатам
      {
          {
     ...
     //  add marker on current position
      ...
      
          {
                 ...
                 currentMarker = new GMarkerGoogle(MainMap.Position, GMarkerGoogleType.arrow);
                  ...
                  //if(false)
                  {
                      ...
                      GeoCoderStatusCode status = GeoCoderStatusCode.Unknow;
                      {
                          PointLatLng? pos = GMapProviders.GoogleMap.GetPoint("Россия, Москва, остановка Шереметьевская ул., остановка Шереметьевская ул.", out status);   //задание объекта
                         // PointLatLng? pos = GMapProviders.OpenCycleTransportMap.GetPoint("Москва, ашан", out status);   //задание объекта
                          
                          // PointLatLng? pos2 = GMapProviders.OpenStreetMap.GetPoint("Россия, Москва, Октябрьская улица", out status);
                          //PointLatLng? pos = GMapProviders.GoogleMap.GetPoint("Russia, Moscow", out status);   //задание объекта
                          //PointLatLng? pos = GMapProviders.GoogleMap.GetPoint("Lithuania, Vilnius", out status);
      ...
      {
          //Form otrabotka = new otrabotka_oblasti();
      ...
      {
         // Form otrabotka2 = new Source.Otrabotka_postroite();
          //otrabotka2.Show();
      }
          }
      }
