...
namespace GMap.NET.MapProvidert
{
    using System;
    ...
    public abstract class GoogleMapProviderBase :  GMapProvidert, RoutingProvidert, GeocodingProvider, DirectionsProvider
    {
        ...
        #region GMapProvidert Members
        public override Guid Id2
        {
            get
            {
        ...
        GMapProvidert [] overlays;
        public override GMapProvidert [] Overlays
        {
            get
            {
                if (overlays == null)
                    ...
                    if (!string.IsNullOrEmpty(html))
                    {
                        ...
                        if (mat.Success)
                        {
                            ...
                            if (count > 0)
                            {
                                ...
                                GMapProviderst.GoogleMap.Version = ver;
                                GMapProviderst.GoogleChinaMap.Version = ver;
#if DEBUG
                        ...
                        if (mat.Success)
                        {
                            ...
                            if (count > 0)
                            {
                                ...
                                string old = GMapProviderst.GoogleHybridMap.Version;

                                GMapProviderst.GoogleHybridMap.Version = ver;
                                GMapProviderst.GoogleChinaHybridMap.Version = ver;
#if DEBUG
                        ...
                        if (mat.Success)
                        {
                            ...
                            if (count > 1)
                            {
                                ...
                                GMapProviderst.GoogleTerrainMap.Version = ver;
                                GMapProviderst.GoogleChinaTerrainMap.Version = ver;
#if DEBUG
        ...
        public MapRoute GetRoute(PointLatLng start, PointLatLng end, bool transit, int Zoom)
        {
            string tooltip;
            int numLevels;
            int zoomFactor;
            MapRoute ret = null;
            List<PointLatLng> points = GetRoutePoints(MakeRouteUrl(start, end, LanguageStr, transit), Zoom, out tooltip, out numLevels, out zoomFactor);
            if (points != null)
            {
        ...
        public MapRoute GetRoute(string start, string end, bool transit, int Zoom)
        {
            string tooltip;
            int numLevels;
            int zoomFactor;
        ...
        string MakeRouteUrl(PointLatLng start, PointLatLng end, string language, bool transit)
        {
            string opt = transit ? RouteWithoutHighwaysStr : RouteStr;
        ...
        string MakeRouteUrl(string start, string end, string language, bool transit)
        {
            string opt = transit ? RouteWithoutHighwaysStr : RouteStr;
        ...
        static readonly string RouteStr = "&mra=ls&dirflg=d"; //d - водитель, r-городской транспорт
        static readonly string Transit = "&mra=ls&dirflg=r";
      ///  static readonly string transitStr = "&mra=ls&dirflg=r";

       // https://www.google.com/maps?ll=40.748441,-73.985664&z=13&t=m&hl=ru-RU&gl=US&mapclient=embed&daddr=Empire+State+Building+350+5th+Ave+New+York,+NY+10118@40.7484405,-73.98566439999999
       // http://maps.google.ru/maps?f=d&source=s_d&saddr=%D0%9C%D0%BE%D1%81%D0%BA%D0%B2%D0%B0+%D1%81%D1%82.+%D0%BC.+%D0%BB%D0%B5%D0%BD%D0%B8%D0%BD%D1%81%D0%BA%D0%B8%D0%B9+%D0%BF%D1%80%D0%BE%D1%81%D0%BF%D0%B5%D0%BA%D1%82&daddr=%D0%9C%D0%BE%D1%81%D0%BA%D0%B2%D0%B0,+%D1%83%D0%BB.+%D0%9A%D0%B0%D1%80%D1%8C%D0%B5%D1%80,+%D0%B4%D0%BE%D0%BC+2%D0%90&geocode=%3BFbfWUQMdoZg9Aik750bHoUy1RjFVO7o6LQHjWA&hl=ru&mra=ls&dirflg=r&ttype=dep&date=06%2F09%2F10&time=09:15&noexp=0&noal=0&sort=&sll=57.77238,33.896433&sspn=5.802708,14.39209&ie=UTF8&z=14&start=0
        ...
        public DirectionsStatusCode GetDirections(out GDirections direction, PointLatLng start, PointLatLng end, bool transit, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, end, LanguageStr, transit, sensor, metric), out direction);
        }

        public DirectionsStatusCode GetDirections(out GDirections direction, string start, string end, bool transit, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, end, LanguageStr, transit, sensor, metric), out direction);
        }
        ...
        /// <returns></returns>
        public IEnumerable<GDirections> GetDirections(out DirectionsStatusCode status, string start, string end, bool transit, bool sensor, bool metric)
        {
        ...
        /// <param name="metric"></param>
        /// <returns></returns>
        public IEnumerable<GDirections> GetDirections(out DirectionsStatusCode status, PointLatLng start, PointLatLng end, bool transit, bool sensor, bool metric)
        {
            // TODO: add alternative directions
        ...
        public DirectionsStatusCode GetDirections(out GDirections direction, PointLatLng start, IEnumerable<PointLatLng> wayPoints, bool transit, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, wayPoints, LanguageStr, transit, sensor, metric), out direction);
        }

        public DirectionsStatusCode GetDirections(out GDirections direction, string start, IEnumerable<string> wayPoints, bool transit, bool sensor, bool metric)
        {
            return GetDirectionsUrl(MakeDirectionsUrl(start, wayPoints, LanguageStr, transit, sensor, metric), out direction);
        }
        ...
        string MakeDirectionsUrl(PointLatLng start, PointLatLng end, string language, bool transit, bool sensor, bool metric)
        {
            ...
            //string at = "&mode=" + (walkingMode ? "walking" : "driving"); // 8
        ...
        string MakeDirectionsUrl(string start, string end, string language, bool transit, bool sensor, bool metric)
        {
        ...
        string MakeDirectionsUrl(PointLatLng start, IEnumerable<PointLatLng> wayPoints, string language, bool transit, bool sensor, bool metric)
        {
            //string av = (avoidHighways ? "&avoid=highways" : string.Empty) + (avoidTolls ? "&avoid=tolls" : string.Empty); // 6
            //string mt = "&units=" + (metric ? "metric" : "imperial"); // 7
            //string wk = "&mode=" + (walkingMode ? "walking" : "driving"); // 8
        ...
        string MakeDirectionsUrl(string start, IEnumerable<string> wayPoints, string language, bool transit, bool sensor, bool metric)
        {
            //string av = (avoidHighways ? "&avoid=highways" : string.Empty) + (avoidTolls ? "&avoid=tolls" : string.Empty); // 6
            //string mt = "&units=" + (metric ? "metric" : "imperial"); // 7
            //string wk = "&mode=" + (walkingMode ? "walking" : "driving"); // 8
    ...
    /// GoogleMap provider
    /// </summary>
    public class GoogleMapProvidert : GoogleMapProviderBase
    {
        public static readonly GoogleMapProvidert Instance;

        GoogleMapProvidert()
        {
        }

        static GoogleMapProvidert()
        {
            Instance = new GoogleMapProvidert();
        }
        ...
        readonly Guid id2 = new Guid("D7287DA0-A7FF-405F-8166-B6BAF26D066C");
        public override Guid Id2
        {
            get
            {
